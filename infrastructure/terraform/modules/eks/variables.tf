...
variable "private_app_subnet_ids" {
  description = "Private application subnet IDs for managed nodes."
  type        = list(string)

  validation {
    condition     = length(var.private_app_subnet_ids) >= 2 && alltrue([for id in var.private_app_subnet_ids : can(regex("^subnet-", id))])
    error_message = "private_app_subnet_ids must contain at least two subnet IDs."
  }
}

variable "cluster_endpoint_public_access" {
  description = "Whether the EKS public API endpoint is enabled during bootstrap."
  type        = bool
  default     = true
}
...
variable "node_group_name" {
  ...
  type        = string
  default     = "eventpulse-dev-general"

  validation {
    condition     = can(regex("^[A-Za-z][A-Za-z0-9-]{2,99}$", var.node_group_name))
    error_message = "node_group_name must start with a letter and contain only letters, numbers, and hyphens."
  }
}
...
variable "addons" {
  description = "Pinned EKS add-on versions by add-on name."
  type        = map(string)

  validation {
    condition     = alltrue([for addon_name in ["vpc-cni", "coredns", "kube-proxy", "eks-pod-identity-agent"] : contains(keys(var.addons), addon_name)])
    error_message = "addons must include vpc-cni, coredns, kube-proxy, and eks-pod-identity-agent."
  }
}
